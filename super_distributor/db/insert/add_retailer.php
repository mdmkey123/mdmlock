...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $query_super_distributor = "SELECT super_admin_id, main_admin_id FROM super_distributor WHERE id = '$super_distributor_id' LIMIT 1";
    ...
    if (!$result_super_distributor || mysqli_num_rows($result_super_distributor) == 0) {
        echo json_encode(['status' => 'error', 'message' => 'Super Distributor not found']);
        exit;
    }

    $super_distributor_data = mysqli_fetch_assoc($result_super_distributor);
    $super_admin_id = $super_distributor_data['super_admin_id'];
    $main_admin_id = $super_distributor_data['main_admin_id'];
