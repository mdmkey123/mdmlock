...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $query = "SELECT id FROM enrollment_keys 
              WHERE assigned_super_distributor = $super_distributor_id 
    ...
    if ($result->num_rows >= $amount) {
        ...
        $update_query = "UPDATE enrollment_keys SET $column_name = $user_id WHERE id IN ($key_ids_str) AND assigned_super_admin = $super_admin_id";
        ...
        $insert_query = "INSERT INTO transaction_history 
            ...
            VALUES 
            ('$transaction_id', '$admin_type', '$user_id', 'credit', '$amount', 'super_distributor', '$super_distributor_id', NOW())";
