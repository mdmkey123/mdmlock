...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($super_admin_id === 0 || empty($admin_type) || $user_id === 0 || $amount <= 0) {
        $response['message'] = 'Invalid input parameters';
    } else {
        $column_mapping = [
            'main_admin' => 'assigned_admin',
        ...
        if ($admin_type === 'main_admin') {
            $null_condition = "AND assigned_super_distributor IS NULL AND assigned_distributor IS NULL AND assigned_retailer IS NULL";
        } elseif ($admin_type === 'super_distributor') {
            $null_condition = "AND assigned_distributor IS NULL AND assigned_retailer IS NULL";
        } elseif ($admin_type === 'distributor') {
            $null_condition = "AND assigned_retailer IS NULL";
        } elseif ($admin_type === 'admin') {
            $null_condition = ""; // No lower level, just revert
        }
        ...
        if ($result && $result->num_rows >= $amount) {
            ...
            if ($master_conn->query($update_query)) {
                // Generate transaction ID (Uses $conn)
                $date_code = date("ymd");
                ...
                // Insert into transaction history (debit from admin, credit to super_admin)
                $insert_query = "INSERT INTO transaction_history 
                    ...
                    VALUES 
                    ('$transaction_id', '$admin_type', '$user_id', 'debit', '$amount', 'super_admin', '$super_admin_id', NOW())";
