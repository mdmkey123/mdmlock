...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($super_admin_id === 0 || empty($admin_type) || $user_id === 0 || $amount <= 0) {
        ...
        // Fetch available keys from enrollment_keys (Uses $master_conn)
        ...
        if ($result->num_rows >= $amount) {
            ...
            $update_query = "UPDATE enrollment_keys SET $column_name = $user_id WHERE id IN ($key_ids_str)";
            ...
            if ($master_conn->query($update_query)) {
                ...
                // Insert into transaction history (Uses $conn)
                $insert_query = "INSERT INTO transaction_history 
                    ...
                    VALUES 
                    ('$transaction_id', '$admin_type', '$user_id', 'credit', '$amount', 'super_admin', '$super_admin_id', NOW())";
                ...
                mysqli_rollback($master_conn);
                mysqli_rollback($conn);
                $response['message'] = 'Failed to update key assignments';
            }
        } else {
            mysqli_rollback($master_conn);
            mysqli_rollback($conn);
